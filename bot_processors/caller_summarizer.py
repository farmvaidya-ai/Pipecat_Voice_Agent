...
from bot_processors.caller_memory import load_latest_summary, save_summary
...
SUMMARY_INSTRUCTION = (
    ...
    "Name: <caller's name only, transliterated into Telugu script — this is a "
    "Telugu-speaking IVR and the name is spoken aloud inside Telugu sentences, "
    "so it must never be left in Latin/English letters (write అర్జున్, not "
    "Arjun) and must never carry an English gloss in parentheses (write "
    "కవిత, not కవిత (Kavitha)); or unknown if never stated>\n"
    "LastTopic: <a short phrase (5-10 words), in Telugu, naming what THIS "
    "latest call specifically was mainly about — not the caller's whole "
    "history, just this one call>\n"
    f"Summary: <the updated summary, at most {SUMMARY_MAX_WORDS} words>"
)
...
GREETING_INSTRUCTION = (
    ...
    "broader summary of their past calls. Write ONE short natural spoken Telugu "
    ...
    "(e.g. మీతో మళ్ళీ మాట్లాడటం సంతోషంగా ఉంది).\n"
    "3. Explicitly say you discussed something before, in your own words, not "
    "read verbatim (e.g. మనం గతంలో ... గురించి మాట్లాడుకున్నాం కదా). Always reference "
    "the given MOST RECENT topic specifically — never a different, older topic "
    "from the broader summary, even if that older topic takes up more of the "
    "summary's length. If no most-recent topic was given, reference the broader "
    "summary's overall topic instead.\n"
    "4. Ask how you can help them today.\n"
    "All 4 parts must be present, in this order, as one flowing spoken line — "
    "do not drop the pleasantry to save words. Plain text, no markdown, no "
    "quotes — output only the line to be spoken aloud."
)
...
async def summarize_and_save_call(llm, context, caller_number: str, call_id: str = "") -> None:
...
async def build_llm_greeting(llm, name: str, summary: str, last_topic: str = "") -> str | None:
