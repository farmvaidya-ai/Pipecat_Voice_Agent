...
Unlike bot_processors.price_lookup, this fetches live, per-call — both IMD
...
Unlike RAGInjector (bot_processors/rag.py) and PriceLookupInjector
(bot_processors/price_lookup.py), this is not a FrameProcessor that injects
...
async def fetch_weather(location: str) -> dict | None:
    ...
    asyncio.to_thread — same pattern as bot_processors/escalation.py's
    outbound HTTP call.
...
def make_get_weather(serializer=None):
    ...
    (bot_processors/call_db.py) — Bot.py builds one LLMContext/serializer per
