...
bot_processors.pricing.agmarknet_scraper's scheduled background job, which
...
one-time scrape (bot_processors.pricing.agmarknet_scraper.scrape_single) — see that
...
from bot_processors.pricing.agmarknet_scraper import (
...
from bot_processors.pricing.price_shared import _LAST_KNOWN_KEY_SEP, crop_keyword_for, normalize_commodity_name
...
# bot_processors.pricing.agmarknet_scraper.harvest_commodity_reference() walks the
...
def make_get_price(serializer=None):
    ...
    (bot_processors/calls/call_db.py) — same pattern as
    bot_processors.location.weather_lookup.make_get_weather.
    """
